...
API_URL = "https://glitch-lyart.vercel.app/api/transfer"
HASHES = [
...
for hash_value in HASHES:
    payload = {"hash": hash_value, "amount": 20000000}
    headers = {"Authorization": "Bearer YOUR_SECRET_TOKEN"}
    
    response = requests.post(API_URL, json=payload, headers=headers)
    print(f"Generated Assembly Instruction for {hash_value}: {response.json()['instruction']}")
